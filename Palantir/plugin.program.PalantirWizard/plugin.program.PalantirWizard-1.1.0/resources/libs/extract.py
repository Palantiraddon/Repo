...
def allWithProgress(_in, _out, dp, ignore, title):
	...
	for item in zin.infolist():
		...
		line2  = '[COLOR %s][B]Archivos:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR] ' % (COLOR2, COLOR1, count, int(nFiles))
		...
		if skip == True: wiz.log("Skipping: %s" % item.filename, xbmc.LOGNOTICE)
			...
			try:
				...
				errormsg  = "[COLOR %s]Archivos:[/COLOR] [COLOR %s]%s[/COLOR]\n" % (COLOR2, COLOR1, file[-1])
				...
				wiz.log('Error al extraer: %s(%s)' % (item.filename, str(e)), xbmc.LOGERROR)
				pass
		dp.update(prog, line1, line2, line3)
		if dp.iscanceled(): break
	if dp.iscanceled(): 
		dp.close()
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Extracion Cancelada[/COLOR]" % COLOR2)
		sys.exit()
